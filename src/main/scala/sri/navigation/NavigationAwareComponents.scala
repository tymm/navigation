...
@js.native
trait NavigatorScreenProps[S <: ScreenClass] extends ReactJSProps {
  val navigation: NavigationScreenProp[S] = js.native
  val router: NavigationRouter = js.native
}

@ScalaJSDefined
trait ScreenClass extends js.Object {
...
S <: AnyRef](implicit ev: =:!=[Params, Nothing])
    extends ComponentJS[NavigatorScreenProps[NavigationRoute[Params]], S]
    with ScreenClass {
